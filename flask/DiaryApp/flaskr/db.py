...
def get_db():
    if 'db' not in g:
        ...
        g.db.row_factory = sqlite3.Row # each row returned by a query will be of type sqlite3.Row, which behaves like both a tuple and a dictionary. This allows you to access columns by name (e.g., row['column_name']) or by index (e.g., row[0]) so basically we can use whichever is convinient whereas without usng row facory we will have to do with only index..
...
def init_db():
    ...
    with current_app.open_resource('schema.sql') as f: #Using current_app.open_resource allows you to open files relative to the application context, making it easier to work with application-specific resources.
