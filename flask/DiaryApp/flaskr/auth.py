...
def login():
    if request.method == 'POST':
        ...
        error = None
        user = db.execute(
            'SELECT * FROM user WHERE username = ?', (username,) #The comma after username in the tuple (username,). without the trailing comma it would be just a string or other variable type
        ).fetchone()

        if user is None:
            error = 'Incorrect username.'
        elif not check_password_hash(user['password'], password): #false na true madi error display madta irodh
